...
// 단위 테스트 (Controller 관련 로직만 띄우기) ex.Filter, ControllerAdvice
...
public class BookControllerUnitTest {
	...
	public void save_테스트() throws Exception {
		// given (테스트를 하기 위한 준비)
		Book book = new Book(null, "스프링 무작정 따라하기", "JG");
		String content = new ObjectMapper().writeValueAsString(book);
		when(bookService.저장하기(book)).thenReturn(new Book(1L, "스프링 무작정 따라하기", "JG"));
		...
		// then (검증)
		resultAction.andExpect(status().isCreated()).andExpect(jsonPath("$.title").value("스프링 무작정 따라하기"))
	...
	public void findAll_테스트() throws Exception {
		...
		books.add(new Book(1L, "스프링부트 따라하기", "스시"));
		books.add(new Book(2L, "리액트 따라하기", "초밥"));
	...
	public void update_테스트() throws Exception {
		...
		when(bookService.수정하기(id, book)).thenReturn(new Book(1L, "C++ 따라하기", "JG"));
